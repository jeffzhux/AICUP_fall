...
@torch.no_grad()
def iterate_data_mean_std(model, dataloader, dataset, cfg):

    idx_feat = {i:[] for i in range(33)}
    feat_mean = {}
    feat_std = {}
    # forward
    for idx, (images, labels) in enumerate(dataloader):
        ...
        feat = model.module.backbone.features(images)
        feat = model.module.backbone.avgpool(feat)

        for i in range(33):
            idx_feat[i].append(feat[labels == i])
        
    for i in range(33):
        idx_feat[i] = torch.cat(idx_feat[i])
        feat_mean[i] = idx_feat[i].mean(0).view(-1)
        feat_std[i] = idx_feat[i].std(0).view(-1)

    torch.save(feat_mean, os.path.join(cfg.work_dir, f'feat_mean.pt'))
    torch.save(feat_std, os.path.join(cfg.work_dir, f'feat_std.pt'))

@torch.no_grad()
def iterate_data(model, dataloader, tta, cfg):

    feat_mean = torch.load(os.path.join(cfg.work_dir, f'feat_mean.pt'), map_location='cuda')
    feat_std = torch.load(os.path.join(cfg.work_dir, f'feat_std.pt'), map_location='cuda')

    feat_mean = torch.stack([v for k, v in feat_mean.items()], dim=0)
    ...
    for idx, (images, labels) in enumerate(dataloader):
        ...
        print(feat.shape)
        feat_l2 = F.normalize(feat, p=2, dim=1)
        print(feat_l2.shape)
        
        id_logits = model.module.backbone.classifier(id_feat)
        ood_logits = model.module.backbone.classifier(ood_feat)
        #logits = tta(logits)

        id_pred_class.append(id_logits)
        id_target.append(labels[labels != 19])

        ood_pred_class.append(ood_logits)
        ood_target.append(labels[labels == 19])

        id_conf = temper * (torch.logsumexp(id_logits / temper, dim=1))
        ood_conf = temper * (torch.logsumexp(ood_logits / temper, dim=1))

        id_confs.extend(id_conf.cpu().numpy())
        ood_confs.extend(ood_conf.cpu().numpy())
        
    id_pred_class = torch.cat(id_pred_class)
    id_target = torch.cat(id_target)

    ood_pred_class = torch.cat(ood_pred_class)
    ood_target = torch.cat(ood_target)

    return id_confs, ood_confs

@torch.no_grad()
def run_eval(model, test_loader, tta, cfg):
    id_confs, ood_confs = iterate_data(model, test_loader, tta, cfg)
    plt.hist(id_confs, alpha=0.5, color='red', bins=50)
    plt.hist(ood_confs, alpha=0.5, color='blue', bins=50)
    plt.savefig('bar25.png')
    exit()
...
def main_worker(rank, world_size, cfg):
    ...
    model.eval()
    if not os.path.exists(os.path.join(cfg.work_dir, 'feat_mean.pt')):
        iterate_data_mean_std(model, train_loader, train_set, cfg)
