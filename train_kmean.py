...
from utils.util import AverageMeter, TrackMeter, accuracy, adjust_learning_rate, format_time, set_seed, Metric
...
def update(model, dataloader, dataset, cfg):
    model.eval()
    features = []
    ...
    with torch.no_grad():
        for idx, (images, targets, loc) in enumerate(dataloader):
            ...
            # forward
            x = model.module.backbone.features(images)
            x = model.module.backbone.avgpool(x)
            x = torch.flatten(x, 1)
            features.append(x)
            
        features = torch.cat(features, dim=0)
        kmean = KMEANS(n_clusters = cfg.num_extra_others+1, verbose=False)
        kmean.fit(features)
        other_idx, _ = zip(*dataset.extra_others_targets)
        
    return list(zip(other_idx, kmean.labels.cpu().tolist()))
...
def valid(model, dataloader, dataset, criterion, optimizer, epoch, cfg, logger, writer):
    ...
    pred_class = torch.cat((
        pred_class[:,:cfg.num_classes-1], 
        torch.sum(pred_class[:,cfg.num_classes-1:], dim=-1, keepdim=True)
    ), dim=-1)
    
    labels = torch.cat(labels)
    labels = torch.cat((
        labels[:,:cfg.num_classes-1], 
        torch.sum(labels[:,cfg.num_classes-1:], dim=-1, keepdim=True)
    ), dim=-1)
...
def main_worker(rank, world_size, cfg):
    ...
    others_sampler = build_sampler(train_set, cfg.data.others_sampler)
    others_loader = torch.utils.data.DataLoader(
        train_set,
        batch_size=cfg.bsz_gpu,
        num_workers=cfg.num_workers,
        collate_fn = train_collate,
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if model_ema:
            valid(model_ema, valid_loader, valid_set, criterion, optimizer, epoch, cfg, logger, writer)
        else:
            valid(model, valid_loader, valid_set, criterion, optimizer, epoch, cfg, logger, writer)
        
        train_set.extra_others_targets = update(model, others_loader, train_set, cfg)
